...
| 可续传归档工作流 | `archive.upload-populate-compress` | 核对偏移量与 HEAD 续传状态；拒绝旧偏移后继续正确上传；流式 SHA256 与本地字节一致；验证发布和下载、真实 ZIP 解压与 7z 压缩、任务终态，以及摘要和详情的边界。 |
| 归档状态与管理 | `archive.upload-state-and-management` | 拒绝非法初始化与分块；验证上传不完整、校验不匹配、取消和失败上传不发布；核对覆盖后的字节、目录与文件增删改查、列表隔离和路径边界。 |
