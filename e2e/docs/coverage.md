...
本清单将已部署后端的各个功能域映射到 `suites/catalog.go` 中具有稳定 ID 的测试用例。当前共 **84 个场景：81 个常规回归用例（其中包含 13 个冒烟用例），以及 3 个需要显式选择的外部服务用例**。
...
| 文件路径边界 | `files.path-confinement` | 在读取、下载、搜索、创建、写入、删除、重命名、上传中拒绝目录穿越和符号链接越界；断开的符号链接不阻断正常目录列表；非法 multipart 路径不造成部分写入，已有安全文件内容保持不变。 |
...
| 安全快照事件后的立即断流 | `world.safety-snapshot-disconnect` | 在真实 SSE 首次提供非空安全快照 ID 时立即关闭连接，不等待后续 Restic 阶段、不重试；精确关联恢复历史与 journal，要求 interrupted 或抢先完成的 succeeded 终态、结束时间、停止写入者及维护释放，拒绝失败或悬挂历史。允许数据已完成而最终响应前断开的请求 journal 保守记录 interrupted，分别记录两者状态；随后真实修改并读取动态配置、创建并暂停计划任务，证明独立 SQLite 写入仍可用；再次改变区域字节后，通过安全回滚精确恢复断流请求之前的数据，无需重启应用。数据库事务内取消时点由后端确定性测试覆盖。 |
