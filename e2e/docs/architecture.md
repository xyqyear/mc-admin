...
The lifecycle is setup → case → case compensation → reuse verification → retirement/next case. Each stage has structured failure reporting. Setup and execution honor their respective context deadlines. Cleanup uses a fresh bounded context even after cancellation. All network calls, subprocesses and polling must propagate context; a Go callback that ignores cancellation cannot be forcibly interrupted by the runner, so CI job timeouts remain the outer limit. Fatal HTTP errors stop polling; only an expected not-yet-ready state is retried. A failed scenario is never automatically rerun.
