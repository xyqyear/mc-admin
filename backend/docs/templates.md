...
- **Direct mode**: the server's on-disk Compose YAML is authoritative. No template association.

`app.configuration.preparation` prepares immutable YAML and matching snapshot/variables for server creation, snapshot editing, and explicit source-template conversion. Authored values override snapshot defaults, and the prepared result serializes its metadata so later form or snapshot mutation cannot change the accepted operation. `POST /servers/{id}/template-config/preview` uses the same snapshot preparation as saving; deleting or editing the source template does not change this preview. The GET preview endpoint describes the server's editing mode.

`routers/servers/template_migration.py` exposes conversion intents. Direct → template uses `extract_variables_from_compose`; if the rendered YAML matches semantically (`are_yaml_semantically_equal()` ignores formatting), the conversion saves metadata without rebuilding. Template → direct clears the association without changing Compose. Both metadata-only paths retain a maintenance lease and durable `configuration_apply` record. Explicit upgrades capture the live template at submission; `TemplateSnapshot.source_updated_at` records that source version separately from snapshot capture time. Legacy snapshots lacking this optional field retain the capture-time comparison.

`app.configuration.application` owns configuration application within the existing task manager. Necessary metadata saving finishes before the task can complete, and before restarting the container. A startup failure reports task failure while the saved metadata still describes the applied Compose. A metadata save failure prevents startup and retains recovery evidence for the file/source mismatch; it does not roll back written Compose automatically. Callers import preparation and application directly from `app.configuration`.

Configuration reads, extraction and conversion checks return an opaque version covering the server generation, canonical Compose target, raw bytes and source metadata. Saves and conversions accept optional `expected_version`; stale requests receive a structured 409, while conflicts discovered after task acceptance appear as failed tasks with `error_code="configuration_conflict"`. Legacy requests can omit the version. The application-level operation observer refreshes actual configuration and status independently of the editor page, while draft state remains separate. See [configuration application](configuration.md) for atomic staging, recovery and external-edit limitations.
