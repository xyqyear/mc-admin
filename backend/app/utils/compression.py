...
def _generate_archive_filename(server_name: str, relative_path: Optional[str] = None) -> str:
    """
...
async def create_server_archive(server_name: str, server_project_path: Path, relative_path: Optional[str] = None) -> str:
    """
    ...
    archive_base_path = settings.archive_path
    ...
    try:
        ...
        if not await aioos.path.exists(archive_path):
            raise HTTPException(
                status_code=500,
                detail="Archive was not created successfully"
            )
        ...
        if "Permission denied" in error_msg:
            raise HTTPException(
                status_code=403,
                detail="Permission denied while creating archive"
            )
        elif "No space left on device" in error_msg:
            raise HTTPException(
                status_code=507,
                detail="Insufficient disk space to create archive"
            )
        elif "7z: command not found" in error_msg or "No such file or directory" in error_msg:
            raise HTTPException(
                status_code=500,
                detail="7z command not available on system"
            )
        else:
            raise HTTPException(
                status_code=500,
                detail=f"Failed to create archive: {error_msg}"
            )