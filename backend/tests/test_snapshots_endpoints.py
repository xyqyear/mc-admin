...
"""
        ...
        (self.data_path / "paper.yml").write_text(
            """settings:
...
"""
        ...
        (self.data_path / "ops.json").write_text("[]")
        (self.data_path / "whitelist.json").write_text("[]")
        (self.data_path / "banned-players.json").write_text("[]")
        (self.data_path / "banned-ips.json").write_text("[]")

        # Create world directory with typical MC world structure
        world_path = self.data_path / "world"
        ...
        plugins_path = self.data_path / "plugins"
...
class TestSnapshotEndpoints:
    ...
    async def test_snapshot_path_filtering_endpoint(
        ...
        with mock_snapshot_dependencies_setup(instance, initialized_restic_repo):
            ...
            time.sleep(0.2)
            (instance.data_path / "plugins" / "test-plugin.jar").write_bytes(
                b"fake jar content"
            )
