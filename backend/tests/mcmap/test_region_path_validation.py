...
def test_rejects_absolute_path():
    with tempfile.TemporaryDirectory() as d:
        with pytest.raises(HTTPException) as exc:
            _resolve_region_path(Path(d), "/etc/passwd")
...
def test_rejects_parent_traversal():
    with tempfile.TemporaryDirectory() as d:
        ...
        with pytest.raises(HTTPException) as exc:
            _resolve_region_path(Path(d), "../foo")
        assert exc.value.status_code == 400


def test_rejects_data_root():
    with tempfile.TemporaryDirectory() as d:
        # "." resolves to data_path itself
        with pytest.raises(HTTPException) as exc:
            _resolve_region_path(Path(d), ".")
        assert exc.value.status_code == 400
...
def test_rejects_missing_directory():
    with tempfile.TemporaryDirectory() as d:
        with pytest.raises(HTTPException) as exc:
            _resolve_region_path(Path(d), "world/region")
...
def test_rejects_file_not_directory():
    with tempfile.TemporaryDirectory() as d:
        ...
        with pytest.raises(HTTPException) as exc:
            _resolve_region_path(data, "world/region")
        assert exc.value.status_code == 404


def test_accepts_valid_relative_dir():
    with tempfile.TemporaryDirectory() as d:
        data = Path(d)
        (data / "world" / "region").mkdir(parents=True)
        result = _resolve_region_path(data, "world/region")
