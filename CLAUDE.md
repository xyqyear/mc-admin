...
- `.github/workflows/static-checks.yml` runs frontend lint/build and backend Pyright/Ruff on every push.
- `.github/workflows/e2e-tests.yml` builds the application image and standalone runner once, runs three independent API regression shards, always attempts cleanup/diagnostic upload, and audits the case/shard union against the deployed OpenAPI schema. New suite directories do not need matrix entries. Manual dispatch supports smoke, external provider qualification and disabling environment reuse; DNS credentials come from the private `E2E_EXTERNAL_CONFIG` secret.
