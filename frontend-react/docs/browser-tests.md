...
The restore journey holds the real map-status request until the interrupted history row is visible, then releases it and verifies that the same open drawer still offers rollback after the map tabs appear. No map or history response is fabricated. A Query/MSW integration test also drives this transition through the real selection panel, history drawer and rollback stream.
