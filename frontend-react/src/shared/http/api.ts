...
function detailMessage(detail: unknown): string | undefined {
  ...
  if (Array.isArray(detail)) return detail.map(detailMessage).filter(Boolean).join('；') || undefined;
  if (!detail || typeof detail !== 'object') return undefined;
  const value = detail as Record<string, unknown>;
  const message = detailMessage(value.message) || detailMessage(value.msg);
  const errors = detailMessage(value.errors);
  return [message, errors].filter(Boolean).join('；') || undefined;
}

export const getErrorMessage = (error: unknown, fallback = '请求失败'): string => {
  if (!error || typeof error !== 'object') return fallback;
  return detailMessage((error as ApiError).message) || fallback;
};
...
export const shouldRetryQuery = (failureCount: number, error: unknown, limit = 3): boolean => {
  if ((error as ApiError | null)?.code === 'ERR_CANCELED') return false;
  const status = getErrorStatus(error);
  if (status && status >= 400 && status < 500 && status !== 408 && status !== 429) return false;
  return failureCount < limit;
};

export const createApiError = (data: unknown, status?: number, code?: string, fallback?: string): ApiError => {
  const body = data && typeof data === 'object' ? data as Record<string, unknown> : {};
  const detail = body.detail;
  const transportMessage = code === 'ERR_CANCELED' ? '请求已取消'
    : code === 'ECONNABORTED' || code === 'ETIMEDOUT' ? '请求超时，请重试'
      : code === 'ERR_NETWORK' ? '网络连接失败，请检查网络后重试' : undefined;
  const message = detailMessage(detail) || detailMessage(body.message) || transportMessage || (status ? `请求失败（HTTP ${status}）` : fallback || '请求失败');
...
api.interceptors.response.use(
  ...
  (error: AxiosError) => {
    ...
    const apiError = createApiError(error.response?.data, status, error.code, error.message);
