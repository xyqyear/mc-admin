...
import { restorationBindingMessage } from '@/features/world/restore/binding'
...
import { useRestorations } from '@/features/world/restore/queries'
...
> = ({ open, onOpenChange, serverId, serverStopped }) => {
  ...
  const { command, state: rollbackState, start, reset } = useRestorationStream(serverId)
  const rollbackId = command?.kind === 'rollback' ? command.restorationId : null
  ...
  const startRollback = (row: RestorationResponse) => {
    if (row.binding_issue) { toast.error(restorationBindingMessage(row.binding_issue)); return }
    ...
    confirm({
      ...
      variant: 'destructive',
      onConfirm: () => {
        start({ kind: 'rollback', restorationId: row.id })
      },
    })
  }
              ...
              {rows.map((row) => {
                ...
                return (
                  ...
                  >
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {canRollback && (
                          <Button
                            size="sm"
                            variant="outline"
                            disabled={!!row.binding_issue || !serverStopped}
                            title={row.binding_issue ? restorationBindingMessage(row.binding_issue) : undefined}
                    ...
                    {row.binding_issue && <p className="text-xs text-destructive">{restorationBindingMessage(row.binding_issue)}</p>}
                    {row.error_message && (
