...
const DnsManagement: React.FC = () => {
  ...
  const hasActualChanges = (status: DNSStatusResponse | undefined) => {
  ...
  const renderStatusIndicator = () => {
    ...
    if (dnsStatus?.state === 'pending') {
      return <StatusBadge tone="warning" icon={AlertCircle}>等待初始化</StatusBadge>
    }

    if (dnsStatus?.state === 'empty' || dnsStatus?.empty_desired) {
      return <StatusBadge tone="warning" icon={AlertCircle}>无可发布配置，保留现有记录</StatusBadge>
    }

    if (dnsStatus?.state === 'degraded' || dnsStatus?.dns_known === false || dnsStatus?.router_known === false || dnsStatus?.unknown_servers?.length || recordsError || routesError) {
      return <StatusBadge tone="warning" icon={AlertCircle}>同步状态不完整</StatusBadge>
    }
    ...
    if (!dnsStatus.dns_diff || !dnsStatus.router_diff) {
      return <StatusBadge tone="warning" icon={AlertCircle}>同步状态尚未确认</StatusBadge>
    }
  ...
  const renderErrors = () => {
    ...
    const errorMessage = getErrorMessage(statusError, '未知错误')
  ...
  return (
    <div className="space-y-4">
      ...
      {!!dnsStatus?.issues?.length && <Alert variant={dnsStatus.state === 'degraded' ? 'destructive' : 'default'}>
        ...
        <AlertDescription><ul>{dnsStatus.issues.map((issue, index) => <li key={index}>{issue}</li>)}</ul></AlertDescription>
      </Alert>}
      {!!dnsStatus?.unknown_servers?.length && <p className="text-sm text-muted-foreground">待核对的服务器：{dnsStatus.unknown_servers.join('、')}</p>}
      {renderDifferences()}
