...
import { useConfigModules, useModuleConfig, useModuleSchema } from '@/features/settings/queries'
...
const DynamicConfig: React.FC = () => {
  ...
  const editor = useEditorDraft(selectedModule, moduleConfig?.config_data)
  const formData = editor.draft ?? {}
  const setFormData = editor.setDraft
  ...
  const handleReloadConfig = () => {
    confirm({
      ...
      onConfirm: async () => {
        try {
          const refreshedConfig = await refetchConfig({ throwOnError: true })
          if (refreshedConfig.data?.config_data) {
            editor.reset(refreshedConfig.data.config_data)
          }
          toast.info('配置已重新载入')
        } catch (error) {
          toast.error(`重新载入失败: ${getErrorMessage(error)}`)
        }
      },
    })
  }
      ...
      onConfirm: async () => {
        try {
          ...
          if (result.updated_config) {
            editor.reset(result.updated_config)
          }
        } catch {
  ...
  const handleCompareConfig = async () => {
    const toastId = toast.loading('正在获取最新配置...')
    try {
      await refetchConfig({ throwOnError: true })
