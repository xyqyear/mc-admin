...
import ServerTerminal from '@/features/servers/ui/ServerTerminal';
import type { ServerTerminalRef } from '@/features/servers/ui/ServerTerminal';
...
const ServerConsole: React.FC = () => {
  ...
  useEffect(() => {
    if (terminalReady && canConnectWebSocket) {
      const size = terminalRef.current?.getSize();
      if (size) connect(size.cols, size.rows);
    }
    return disconnect;
  }, [terminalReady, canConnectWebSocket, connect, disconnect]);
