...
export function useServerConsoleWebSocket({
  ...
  const buildWebSocketUrl = useCallback(() => {
    ...
    return `${protocol}//${host}/api/servers/${serverId}/console?token=${encodeURIComponent(token)}`
  }, [serverId, token])
  ...
  }, [onLogsUpdate, onLogsRefresh, onCommandResult, onError, onInfo, onAutoScrollEnable])

  // 连接WebSocket
  const connect = useCallback(() => {
    if (!serverId || !token) {
      log.log('Cannot connect WebSocket: missing serverId or token', { serverId, token: !!token })
      return
    }
  ...
  }, [serverId, token, filterRcon, buildWebSocketUrl, sendFilterUpdate, handleMessage])
